...
```{r}
female_filter <- filter(exercise_combine, sex == "F")

female_table <- table(female_filter$romantic, female_filter$reason)
female_table
```
...
Doing the same thing, but subsetting the data this time.
...
Finding the measures of center and spread across a single grouping variable.
...
Finding the measures of center and spread across two grouping variables.
...
Subsetting the data for numeric variables only.
...
Creating a correlation matrix between all numeric variables.
...
```{r}
histogram_absences <- ggplot(exercise_combine, aes(x = absences.x)) + geom_histogram(aes(fill = sex)) + labs(title = "Histogram of Absences by Sex", x = "Absences")
histogram_absences
```
...
```{r}
kernel_absences <- ggplot(exercise_combine, aes(x = absences.x)) + geom_density(alpha = 0.5, aes(fill = sex)) + labs(title = "Kernel Density Plot of Absences by Sex", x = "Absences")
kernel_absences
```
...
```{r}
G3_age_facet <- ggplot(exercise_combine, aes(x = age, y = G3.x, color = sex)) + geom_point() + geom_jitter(width = 0.1, alpha = 0.3) + labs(title = "Scatterplot of G3 and Age by Sex Based on Internet Access", x = "Age", y = "G3") +
  facet_wrap(~ internet)
G3_age_facet
```
...
```{r}
G3_failures_facet <- ggplot(exercise_combine, aes(x = failures, y = G3.x, color = school)) + geom_point() + geom_jitter(width = 0.1, alpha = 0.3) + labs(title = "Scatterplot of G3 and Failures by School Based on Family Size", x = "Failures", y = "G3") +
  facet_wrap(~ famsize)
G3_failures_facet
```
...
```{r}
G3_age_facet2 <- ggplot(exercise_combine, aes(x = age, y = G3.x, color = sex)) + geom_point() + geom_jitter(width = 0.1, alpha = 0.3) + labs(title = "Scatterplot of G3 and Age by Sex Based on School and Internet Access", x = "Age", y = "G3") +
...
```{r}
G3_failures_facet2 <- ggplot(exercise_combine, aes(x = failures, y = G3.x, color = school)) + geom_point() + geom_jitter(width = 0.1, alpha = 0.3) + labs(title = "Scatterplot of G3 and Failures by School and Activities", x = "Failures", y = "G3") +
